...
#### Shell скрипты:  
1) ./terrafotm/terraform_start.sh - запуск терраформирования с указанными параметрами по *.tf файлам текущей директории + запуск add_ip_to_ssh_config.sh . !Вносит наш bastion_netology и обновляет его ip, чистит старые ssh креды наших vm перед их пересозданием!
